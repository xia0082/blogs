...
(function ($) {
    $._ = {
         ...
         * method：方法(默认get)
         * asy：异步(默认true)
         * success：成功的回调函数 Function(http_request.responseText)
         */
        ...
        getRandIntNum: function (min, max) {
         ...
         * callback：回调函数
         */
        jsonpFn: function (url, callbackName, callback, argue) {
            window[callbackName] = callback;
            var script = document.createElement('script');
            var str = Ajax.changeFn(argue);
            script.src = str ? (url + '?' + str) : url;
         ...
         * setp:总位数;
         */
        centerFn: function (sum, num, setp) {
            return sum / 2 - (num * setp) / 2;
        },
        /**
         ...
         * error: 异步失败返回的数据
         */
        asyncFn: function (success, error) {
         ...
         * page: 指定页数
         * num: 每页显示的条数
         * lock: true --> 正序 false --> 倒序
         */
        pageFn: function (data, page = 1, num = 5, lock = true) {
            if (typeof data == 'Array') return 'data应为数组格式';
            let array = [];
            if (lock) {
                ...
                for (let i = data.length - page * num + num - 1; i > data.length - page * num - 1 && i >= 0; i--) {
                    const element = data[i];
                    array.push(element);
                }
            }
